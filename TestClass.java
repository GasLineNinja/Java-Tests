...
public class TestClass extends TestCase{
	...
	public void testDeVoweler() {
		...
		assertEquals(MyClass.deVoweler("abcd", 'a'), "bcd");
		//2. the string is not empty, but does not contain the vowel passed
		assertEquals(MyClass.deVoweler("hjkl", 'u'), "hjkl");
		//3. the string is empty
		assertEquals(MyClass.deVoweler("", 'e'), "");
		//4. the character passed is not a vowel
		assertEquals(MyClass.deVoweler("tkold", 't'), "okld");
	}
	...
	public void testSorter() {
		...
		arrayToSort = new char[]{0, '1', '4', '3', '2'};
		expected = new char[]{0, '1', '2', '3', '4'};
		assertArrayEquals(expected, MyClass.sorter(arrayToSort));
